...
Forward rates are derived from the forward curve using the no-arbitrage condition:

$$F(T_1, T_2) = \frac{\ln\left(\frac{P(0, T_1)}{P(0, T_2)}\right)}{T_2 - T_1}$$

Where $P(0, T)$ is the discount factor from the forward curve (not the discount curve).

### Implementation Details

1. **Cashflow Generation**: 
   - Fixed leg: Uses fixed rate with specified payment frequency (default: semi-annual) and day count (default: 30/360)
   - Floating leg: Uses forward rates from the forward curve with specified payment frequency (default: quarterly) and day count (default: ACT/365)

2. **Discounting**: All cashflows are discounted using the OIS discount curve

3. **Spread Handling**: Optional spread can be added to floating leg forward rates

4. **Direction**: The tool supports both fixed payer and fixed receiver swaps

---

## Input Curves

The tool requires two distinct input curves:

### 1. SONIA OIS Discount Curve

**Purpose**: Used for discounting all cashflows to present value

**Input Format**: CSV file (`data/sonia_ois_quotes.csv`) with columns:
- `instrument_type`: Should be "OIS_MARKET"
- `tenor_years`: Tenor in years (e.g., 1, 2, 3, 5, 10, 15, 20, 30)
- `rate`: Par swap rate (as decimal, e.g., 0.035 for 3.5%)

**Example Data:**
```
instrument_type,tenor_years,rate
OIS_MARKET,1,0.030500
OIS_MARKET,2,0.031000
OIS_MARKET,5,0.033500
OIS_MARKET,10,0.036000
OIS_MARKET,30,0.040000
```

**Market Source**: These are typically SONIA OIS quotes from interdealer brokers or market data providers. OIS rates represent the cost of borrowing/lending overnight funds secured against collateral.

### 2. SONIA Forward Curve (Par Swap Rates)

**Purpose**: Used for projecting floating leg cashflows

**Input Format**: CSV file (`data/sonia_forward_quotes.csv`) with columns:
- `instrument_type`: Should be "SONIA_SWAP"
- `tenor_years`: Tenor in years
- `rate`: Par swap rate (as decimal)

**Example Data:**
```
instrument_type,tenor_years,rate
SONIA_SWAP,1,0.03566
SONIA_SWAP,2,0.03454
SONIA_SWAP,5,0.03528
SONIA_SWAP,10,0.03838
SONIA_SWAP,30,0.04320
```

**Market Source**: These are SONIA swap rates (fixed rates that make a swap have zero value at inception). These rates are typically quoted by market makers and represent the market's expectation of future SONIA rates.

### Why Two Separate Curves?

In the post-LIBOR era, the market has moved to a **multi-curve framework**:
- **OIS Curve**: Reflects the risk-free rate for collateralized transactions (used for discounting)
- **Swap Curve**: Reflects the market's expectation of future floating rates plus credit/liquidity premiums (used for projection)

This separation is essential for accurate pricing, especially for longer-dated swaps where the basis between OIS and swap rates can be significant.
...
The discount curve is constructed from SONIA OIS par swap rates using **coupon stripping** (bootstrapping).

#### Algorithm:

1. **Input**: Par swap rates at various tenors (e.g., 1Y, 2Y, 5Y, 10Y, 30Y)

2. **Payment Frequency**: Assumes quarterly payments (4 per year)

3. **Bootstrapping Process**:
   - For each payment date $T_i$, solve for the discount factor $P(0, T_i)$ such that the par swap has zero value
   - The first discount factor (at 0.25 years) is:
     $$P(0, 0.25) = \frac{1}{1 + R_1 \times 0.25}$$
   - For subsequent periods, use the recursive formula:
     $$P(0, T_i) = \frac{1 - R_i \times \sum_{j=1}^{i-1} \tau_j \times P(0, T_j)}{1 + R_i \times \tau_i}$$
     Where:
     - $R_i$ = Par swap rate interpolated at tenor $T_i$
     - $\tau_j$ = Accrual period for payment $j$

4. **Zero Rate Conversion**: Convert discount factors to continuously compounded zero rates:
   $$r(T) = -\frac{\ln(P(0, T))}{T}$$

5. **Interpolation**: Uses linear interpolation in zero rates for intermediate tenors

6. **Extrapolation**: 
   - For tenors before the first market quote: Uses the first rate
   - For tenors after the last market quote: Extends using the last rate

### Forward Curve Bootstrapping

The forward curve uses the **same bootstrapping methodology** as the discount curve, but from SONIA par swap rates instead of OIS rates.

**Key Difference**: The forward curve is used to extract forward rates between payment dates, while the discount curve is used to discount cashflows.

### Forward Rate Extraction

Once both curves are constructed, forward rates for each floating leg period are calculated as:

$$F(T_{start}, T_{end}) = \frac{\ln\left(\frac{P_{forward}(0, T_{start})}{P_{forward}(0, T_{end})}\right)}{T_{end} - T_{start}}$$

Where $P_{forward}$ is the discount factor from the **forward curve** (not the discount curve).

### Implementation Notes

- **Interpolation Method**: Linear interpolation in zero rates
- **Payment Frequency**: Both curves assume quarterly payments (4 per year) for bootstrapping
- **Day Count**: Uses actual day counts (ACT/365) for time calculations
- **Continuously Compounded**: All rates are stored as continuously compounded rates internally

---

## Model Assumptions

### 1. **No-Arbitrage Framework**
- The model assumes no arbitrage opportunities exist in the market
- Forward rates are derived from discount factors using no-arbitrage relationships

### 2. **Deterministic Interest Rates**
- Interest rates are treated as deterministic (no stochastic modeling)
- No volatility or correlation modeling for rates
- Suitable for pricing but not for exotic options or path-dependent structures

### 3. **Linear Interpolation**
- Zero rates are interpolated linearly between market quotes
- This is a simple but widely used approach
- **Limitation**: May not capture curve shape accurately between quotes, especially in steep or inverted yield curve environments

### 4. **Constant Extrapolation**
- Rates beyond the last market quote are assumed constant (flat extrapolation)
- **Limitation**: May not reflect market expectations for very long-dated cashflows

### 5. **No Credit Risk**
- The model assumes counterparties are risk-free (or credit risk is captured in the curves)
- No CVA (Credit Valuation Adjustment) or DVA (Debit Valuation Adjustment)

### 6. **No Funding Costs**
- Assumes funding costs are embedded in the discount curve
- No separate funding spread modeling

### 7. **Perfect Collateralization**
- Assumes swaps are fully collateralized
- Collateral rate is embedded in the OIS discount curve

### 8. **No Basis Risk Between Curves**
- Assumes the basis between OIS and swap curves is static
- Does not model basis volatility

### 9. **Fixed Payment Schedules**
- Payment frequencies are fixed (not floating based on market conditions)
- Day count conventions are fixed (30/360 for fixed leg, ACT/365 for floating leg)

### 10. **No Optionality**
- The model prices vanilla swaps only
- No embedded options, cancellations, or early termination features

### 11. **Instantaneous Curve Shifts**
- Stress tests assume instantaneous, parallel, or non-parallel shifts
- Does not model gradual rate changes over time

### 12. **Single Currency**
- Currently supports GBP (SONIA) only
- No cross-currency swap capabilities
